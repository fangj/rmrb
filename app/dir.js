...
async function working(){
  ...
  for(let i=0;i<forums.length;i++){//一个月
    ...
    const lines=threads.map(thread=>thread.postdate+" "+thread.subject);
    const dir=lines.join("\r\n");
    const text="### "+forum.name+"\r\n"+dir;
    const filename=path.join(__dirname,"..","dir",forum.name+".md");
